...
ALWAYS_INLINE __u32 crc32_c(const __u8 *data, __u32 len, __u32 init) {
    __u32 crc = init;
...
ALWAYS_INLINE __u32 gen_cookie() {
    return bpf_get_prandom_u32();
}

#define MCRR_SIGNATURE_SIZE  (4)
#define MIN_SECKET_KEY_LEN  (8)
#define MAX_SECKET_KEY_LEN  (26)
#define HASH_SIZE  (4)
#define COOKIE_SIZE  (4)
#define OFFSET_HASH1  (8)
#define OFFSET_HASH2  (OFFSET_HASH1 + HASH_SIZE)

ALWAYS_INLINE int handle_MCRR_reply(
    struct ethhdr *eth, 
    struct iphdr *ip, 
    struct udphdr *udp,
    ...
    if (!secret_key || secret_key_len < MIN_SECKET_KEY_LEN || secret_key_len > MAX_SECKET_KEY_LEN) {
        return XDP_PASS;
    }

    if (payload + MCRR_SIGNATURE_SIZE + COOKIE_SIZE + 2 * HASH_SIZE > payload_end) {
        return XDP_PASS;
    }

    if (payload + MCRR_SIGNATURE_SIZE + MAX_SECKET_KEY_LEN > payload_end) {
        return XDP_PASS;
    }

    __u8* cookie_bytes = &payload[COOKIE_SIZE];

    __u32 hash1;
    __builtin_memcpy(&hash1, &payload[OFFSET_HASH1], HASH_SIZE);

    __u32 hash2;
    __builtin_memcpy(&hash2, &payload[OFFSET_HASH2], HASH_SIZE);

    __u8 hash1_data[HASH_SIZE + MAX_SECKET_KEY_LEN];
    __builtin_memcpy(hash1_data, cookie_bytes, HASH_SIZE);
    __builtin_memcpy(hash1_data + HASH_SIZE, secret_key, secret_key_len);

    __u32 expected_hash1 = crc32_c(hash1_data, HASH_SIZE + secret_key_len, 0);
    
    __u32 expected_hash2 = crc32_c(cookie_bytes, HASH_SIZE, expected_hash1);

    if (hash1 != expected_hash1 || hash2 != expected_hash2) {
        bpf_printk("MCR: Challenge failed\n");
        return XDP_PASS;
    }
    ...
    compute_udp_checksum(ip, udp, payload_end);
    
    bpf_printk("MCR: Challenge pass!\n");
    return XDP_DROP;
}

ALWAYS_INLINE int handle_MCRC_request(
    struct ethhdr *eth,
    struct iphdr *ip,
    struct udphdr *udp,
    __u8* payload, __u8* payload_end
...
ALWAYS_INLINE int check_MCRH3110_signature(__u8 *payload, __u8* payload_end) {
...
ALWAYS_INLINE int check_MCRR_signature(__u8 *payload, __u8* payload_end) {
    if (payload + 4 > payload_end) {
        return 0;
    }
    return (
